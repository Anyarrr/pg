  ...
  return (
    <section className="py-24 bg-[#F8FAFC] overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-6xl mx-auto bg-white rounded-[4rem] shadow-[0_50px_100px_rgba(0,43,91,0.07)] overflow-hidden border border-gray-100 flex flex-col lg:flex-row">
          ...
          <div className="lg:w-2/3 p-8 md:p-16 border-r border-gray-50">
            ...
            <label className="text-xs font-black uppercase tracking-tighter text-gray-400 mb-6 block">01. Где подключаем?</label>
              <div className="flex gap-4">
                <button 
                  ...
                  className={`flex-1 flex items-center justify-center gap-3 py-5 rounded-2xl font-black transition-all border-2 ${options.type === 'flat' ? 'border-pgBlue bg-pgBlue/5 text-pgBlue shadow-inner' : 'border-gray-100 text-gray-400 hover:border-gray-200'}`}
                >
                  <FiHome size={22} /> Квартира
                </button>
                <button 
                  onClick={() => setOptions({...options, type: 'house'})}
                  className={`flex-1 flex items-center justify-center gap-3 py-5 rounded-2xl font-black transition-all border-2 ${options.type === 'house' ? 'border-pgBlue bg-pgBlue/5 text-pgBlue shadow-inner' : 'border-gray-100 text-gray-400 hover:border-gray-200'}`}
                >
                  <FiGlobe size={22} /> Частный дом
                </button>
              </div>
            </div>
            ...
            <div className="mb-12">
              ...
              <label className="text-xs font-black uppercase tracking-tighter text-gray-400 block">02. Скорость интернета</label>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-black text-green-600 uppercase">Максимальная</span>
                </div>
              </div>
              <div className="bg-pgBlue-dark rounded-[2.5rem] p-10 text-white relative overflow-hidden group">
                <div className="relative z-10">
                  <div className="flex items-baseline gap-3 mb-6">
                    <span className="text-7xl md:text-8xl font-black italic tracking-tighter drop-shadow-lg">1000</span>
                    <span className="text-2xl font-bold text-blue-300">Мбит/с</span>
                  </div>
                  <div className="bg-white/10 backdrop-blur-md p-5 rounded-2xl border border-white/10 max-w-md">
                    ...
                    <ul className="space-y-3">
                      <li className="flex items-center gap-3 text-white font-bold">
                        ...
                        <span className="text-sm">Коннект для 10+ устройств сразу</span>
                      </li>
                      <li className="flex items-center gap-3 text-white font-bold">
                        <FiCheck className="text-pgOrange text-xl" />
                        <span className="text-sm">0% задержки в играх и звонках</span>
                      </li>
                    </ul>
                  </div>
                </div>
                <FiZap className="absolute -right-10 -bottom-10 text-[18rem] text-white/[0.03] rotate-12 group-hover:text-pgOrange/10 transition-colors duration-1000" />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div 
                  ...
                  className={`p-6 rounded-3xl border-2 cursor-pointer transition-all flex items-center gap-4 relative ${options.router ? 'border-pgOrange bg-pgOrange/5' : 'border-gray-100 hover:bg-gray-50'}`}
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${
                      options.router ? 'bg-pgOrange' : 'bg-gray-200 border-2 border-gray-300'
                    }`}>
                      {options.router && (
                ...
                <div 
                  ...
                  className={`p-6 rounded-3xl border-2 cursor-pointer transition-all flex items-center gap-4 relative ${options.box ? 'border-pgOrange bg-pgOrange/5' : 'border-gray-100 hover:bg-gray-50'}`}
                >
                  ...
                  <div className="flex items-center gap-2">
                    <div className="font-black text-pgBlue-dark">+99 ₽</div>
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${
                      options.box ? 'bg-pgOrange' : 'bg-gray-200 border-2 border-gray-300'
                    }`}>
                      {options.box && (
                        <FiCheck className="text-white" size={14} />
                      )}
         ...
         <div className="lg:w-1/3 bg-gray-50 p-8 md:p-12 flex flex-col justify-between">
            ...
            <div className="pt-8 border-t border-gray-200">
              ...
              <button 
                ...
                className="group w-full bg-pgOrange hover:bg-pgOrange-hover text-white font-black py-6 rounded-[2rem] text-xl shadow-2xl shadow-pgOrange/40 transition-all flex items-center justify-center gap-3 active:scale-95"
              >
      ...
      {isModalOpen && (
        <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 transition-all duration-300 ${
          ...
          {/* Overlay */}
          <div 
            className={`absolute inset-0 bg-pgBlue-dark/80 backdrop-blur-md transition-opacity duration-300 ${
              isClosing ? 'opacity-0' : isOpening ? 'opacity-0' : 'opacity-100'
            }`}
            onClick={handleCloseModal}
          ></div>
          
          <div className={`relative w-full max-w-5xl bg-white rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] transition-all duration-300 ${
            isClosing 
            ...
            <div className="md:w-1/2 h-64 md:h-auto bg-gray-100 relative">
               ...
               <div 
                 className={`absolute inset-0 bg-pgBlue-dark/10 backdrop-blur-[2px] pointer-events-none flex items-center justify-center transition-all duration-500 ${
                   address ? 'opacity-0 invisible' : 'opacity-100 visible'
                 }`}
               >
                  <div className={`bg-white/90 p-6 rounded-3xl shadow-xl text-center border border-pgOrange/20 transition-all duration-500 ${
                    address ? 'scale-95 opacity-0' : 'scale-100 opacity-100'
                  }`}>
